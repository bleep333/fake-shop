                ...
                <div className="p-6 space-y-6">
                  {cartItems.map((item, index) => (
                    <div key={`${item.product.id}-${index}`} className="flex gap-4">
                      <div className="relative w-24 h-32 bg-stone-100 rounded overflow-hidden flex-shrink-0">
                        <CartItemImage product={item.product} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="font-medium text-stone-900 mb-1 truncate">{item.product.name}</h3>
                        <p className="text-sm text-stone-500 mb-2">Size: {item.size}</p>
