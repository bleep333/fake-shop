...
export default function NewArrivalsContent() {
  ...
  // Calculate price range and get available colors and categories from all new arrival products
  useEffect(() => {
  ...
  useEffect(() => {
    async function loadProducts() {
      setLoading(true)
      try {
        // Always filter by "New" tag, but also include any additional tags from filters
        const tagsToFilter = ['New']
        if (filters.tags && filters.tags.length > 0) {
          // Add other tags if selected (though for New Arrivals, we primarily show "New")
          filters.tags.forEach(tag => {
            if (tag !== 'New' && !tagsToFilter.includes(tag)) {
    ...
    <div className="w-full py-8">
      ...
      <div className="mb-12 px-4 sm:px-6 lg:px-8">
        ...
        <p className="text-stone-600 text-base">
          Discover our latest additions. Fresh styles and new collections just for you.
        </p>
      </div>
        ...
        <aside className="hidden lg:block flex-shrink-0">
          <FilterSidebar 
            ...
            onFiltersChange={(newFilters) => {
              ...
              router.push(queryString ? `/new-arrivals?${queryString}` : '/new-arrivals', { scroll: false })
            }} 
