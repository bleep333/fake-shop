...
export default function SaleContent() {
  ...
  // Calculate price range and get available colors and categories from all sale products
  useEffect(() => {
  ...
  useEffect(() => {
    async function loadProducts() {
      setLoading(true)
      try {
        // Always filter by "Sale" tag, but also include any additional tags from filters
        const tagsToFilter = ['Sale']
        if (filters.tags && filters.tags.length > 0) {
          // Add other tags if selected (though for Sale, we primarily show "Sale")
          filters.tags.forEach(tag => {
            if (tag !== 'Sale' && !tagsToFilter.includes(tag)) {
  ...
  return (
    <div className="w-full py-8">
      ...
      <div className="mb-12 px-4 sm:px-6 lg:px-8">
        <h1 className="text-4xl font-bold mb-3">Sale</h1>
        <p className="text-gray-600 text-base">
          Shop our exclusive sale collection. Limited time offers on selected items.
        </p>
      </div>
        ...
        <aside className="hidden lg:block flex-shrink-0">
          <FilterSidebar 
            ...
            onFiltersChange={(newFilters) => {
              ...
              router.push(queryString ? `/sale?${queryString}` : '/sale', { scroll: false })
            }} 
